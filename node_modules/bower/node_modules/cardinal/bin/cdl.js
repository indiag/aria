...
function usage() {
  var msg = [ 
      'Usage: cdl <filename.js_old> [options]'
    , ''
    , 'Options (~/.cardinalrc overrides):'
    , '  --nonum: turn off line printing'
    , ''
    , 'Unix Pipe Example: cat filename.js_old | grep console | cdl'
    , ''
  ].join('\n');
  console.log(msg);
}
...
// E.g., "cardinal myfile.js_old"
if (args.length === 3) return highlightFile();

var opt = args[3];

// E.g., "cardinal myfile.js_old --nonum"
...
// UNIX pipes e.g., "cat myfile.js_old | grep console | cardinal
  ...
  .on('data', function (chunk) {
    chunk.split('\n').forEach(function (line) {
      try {
        ...
        // line doesn't represent a valid js_old snippet and therefore cannot be parsed -> just print as is
        stdout.write(line + '\n');
      }
    });
  });
